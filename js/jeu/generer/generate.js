...
function majuscule() {
	document.getElementById('champ_entree').this.value.toUpperCase();
}
...
function validation() {
	...
	var mots_aleatoire = document.getElementById('affichage_mot').innerHTML;
		for (let i = 0; i < mots_aleatoire.length; i++) {
  			if (mots_aleatoire[i].toUpperCase() == mots_user[i]) {
				console.log(mots_aleatoire[i].toUpperCase());
			} else if(mots_aleatoire[i].toUpperCase() != mots_user[i]) {
				console.log(".");

				document.getElementById('mybutton').style.color='#801b0f';
			}
		}
		if (mots_aleatoire.toUpperCase() == mots_user) {
			alert("Win !");
